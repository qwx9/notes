...
Let w the base d equivalent of P[1..m]
...
w can be computed in Θ(m) time using Horner's rule:
	...
	w = P[m] + d * (P[m-1] + d * (P[m-2] + ... + d * (P[2] + d * P[1]) ... ))
...
we have to explicitely compare P and S[t+1..t+m+1].
...
computing a hash of P and S[t+1..t+m] using Horner's rule and modulo q.
...
but mean execution time, averaging some hypothesis, is better.
	...
	if P = x^m and S = x^n, i.e. both have the same values everywhere,
...
Where the brute-force method must compare P and S[j] at every position j,
depending on where the mismatch occurs in P, we may restarting comparing
at an index in P higher than 1.

	If a mismatch occurs between P[i] and S[j+i] (with 1<i≤m),
	then P[1..i-1] = S[j..j+i-1].

	Let U = P[1..i-1] = S[j..j+i-1],
	a = P[i], b = S[j+i], with a ≠ b.

		      v    i
	P´	    [V]c[V]a…        V is a border of U
	P	    [   U ]a…
		    ↑======≠
	S	   …[   U ]b…
		    j      j+i
		           ...
		           i
	P	        [U]c…        we must restart at P[v+1] =? S[j+i]
		           ↑         but there's no border V of U
	S	       …[U]b…
		    j      j+i

		           i
	P	            …        we can restart at P[1] and S[j+i+1]
		            ↑
	S	          …b…
		    j      j+i

Let V be a suffix of U of length v.
It is possible that V is also a prefix of P.
In that case, instead of directly restarting comparison at P[1] and S[j+1],
we may instead restart at P[v] and S[j+i],
skipping this prefix we already know matches in both P and S.

	• j is never decreased, ie. we never go backwards in S
	• if mismatch, we just don't want to start again with P[1] at S[j+1]
	• best case: there is no border ⇒ U can be skipped over entirely
		∗ we know there can be no correspondence in P[1..i]
		∗ skip U over S, start again at P[1] and S[j+i+1]
	• otherwise we must jump backwards in P
		∗ P may still match at S[j+i]
		∗ skip V over P and start again at P[v+1] and S[j+i]
	• if i=1, there can be no border (no proper prefix)
		∗ we note this as -1 in the jump table
		∗ so if i = T[i] = -1, increment i and j to restart at P[1] and S[j+i+1]

We may thus precalculate a jump table T which for each position i in P
yields the length of the largest proper suffix of P[1..i-1] that is also a prefix of P,
corresponding to the index we may jump to, or -1 if there is no such suffix,
in which case we must resume at P[1] and S[i+j+1].
...
A proper suffix of a string is not equal to the string and is non-empty.
	...
	T[i] is the length of the longest border of P[1..i] for 1<i≤m.
...
Since a mismatch at the very start of P is a special case,
where no backtracking is possible,
we just leave T[1] at 0.
For i>1, if there is no border in P[1..i], we set T[i] = 0,
...
Any time there's a match with P[i], its position in S is remembered,
...
Each character in S can be compared to P more than once,
...
It is invariant regardless of how many repetitive elements there are in P or S.
...
We compare P and S right to left.
On mismatch, we look backwards in P for the first occurrence of S[j],
...
In the best case, P is not in S, we would get O(n / m) time,
...
Implementation-wise,
the naive method is to parse P from start to end
and for every character c ∈ Σ,
to find occurrences of c.
Here the size of the table is in the order of m·|Σ|,
since we need all positions of c in P.
In other words, for each position in P,
we have the position of the first occurrence of each c
backwards in P.

A better alternative in both runtime and memory
is to save the position of the last occurrence of c in P,
for each c ∈ Σ.
In this case, on mismatch, we would look for the last of occurrence of c in P,
...
and P matching all the way except its first character,
...
If P is not complex, ie. non-periodic, we could arrive at 3·n (recent).

The lower bound is actually Ω(n/m),
which is the best complexity we could expect if we only preprocess P.
A simple example is P having different characters than S,
esp. when the last character of P ∉ Σ,
...
At a given window,
we check the first, last and middle characters of P in S,
...
and if P is non-periodic, that is without many repeats,
